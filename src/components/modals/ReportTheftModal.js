...
export default function ReportTheftModal({ onClose, preselectedDeviceId = null }) {
  ...
  function handleSubmit() {
    ...
    showToast('Theft report submitted!', 'Police will review and verify within 24 hours.', 'info');
  ...
  if (myRegisteredDevices.length === 0) {
    return (
      <Modal
        title={
          <>
            <FiAlertCircle style={{ marginRight: 8, color: 'var(--red)' }} /> Report Stolen Device
          </>
        }
        onClose={onClose}
      >
        <div className="modal-body">
          <div className="alert alert-amber">
            ...
            <div>
              No registered devices available to report stolen. If your device was stolen before you
              registered it, contact your nearest police station directly.
            </div>
          </div>
        </div>
    ...
    >
      <div className="modal-body">
        ...
        <div className="alert alert-amber">
          ...
          <div>
            Filing a false theft report is a criminal offence under the Communications Act. All
            reports are police-verified before any network alert is activated.
          </div>
        </div>
      </div>
