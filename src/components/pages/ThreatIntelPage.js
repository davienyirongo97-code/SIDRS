...
const HOTSPOT_AREAS = [
  ...
  },
  {
    ...
    reports7d: 3,
    avgTime: '07:00–12:00',
    peak: 'Wednesday market day',
  },
  {
    area: 'Mzuzu City Centre',
    city: 'Mzuzu',
    risk: 45,
    trend: 'down',
    lat: -11.458,
...
const ANOMALIES = [
  {
    ...
    detail:
      'Device detected at Kawale (13.922°S) at 09:14, then Area 1 (13.963°S) at 09:22 — 7km in 8 minutes on foot. Suggests device was transported by vehicle immediately after theft.',
  ...
  },
  {
    ...
    detail:
      'Device connected with IMEI 490123456789012 on 08/03 at 09:14. Same physical SIM then connected with IMEI 490999888777001 on 09/03 at 13:22. IMEI was reprogrammed between connections.',
    action:
      'Original IMEI flagged. New IMEI 490999888777001 automatically added to watch list. Blockchain block #14 records this event as court evidence.',
    confidence: 99,
    lat: -13.9226,
    lng: 33.7641,
  },
  {
    ...
    detail:
      'IMEI 490123456789012 has used 3 different SIM cards in 48 hours: +265 991 887 766 (Airtel), +265 881 223 344 (TNM), +265 991 441 882 (Airtel). Rapid SIM cycling is a known evasion technique.',
    action:
      'All three SIM numbers added to police intelligence. Request subscriber details from Airtel and TNM for each number.',
    confidence: 91,
    lat: -13.9726,
    lng: 33.7841,
  },
  {
    ...
    detail:
      'IMEI 357893109876543 detected simultaneously at Zomba Town Tower (15:3869°S) and Chancellor College Area (15.3769°S) at 08:12. Two devices are broadcasting the same IMEI — one is a clone.',
    action:
      'Flag both devices. Owner of legitimate device (U002) notified. Cloned device SIM number: +265 881 556 677 — request subscriber info from TNM.',
    confidence: 98,
    lat: -15.3869,
    lng: 35.3183,
  },
  {
    ...
    detail:
      'Device registered to owner in Lilongwe (Area 13). IoT node detection at Shoprite City Mall WiFi is consistent — but latest WiFi probe suggests device may be moving toward Blantyre based on tower pattern.',
    action:
      'Monitor. If next detection is in Blantyre, raise to HIGH severity — cross-district transport indicates professional operation.',
    confidence: 74,
    lat: -15.786,
    lng: 35.003,
  },
...
const CASE_SCORES = [
  {
    ...
    score: 92,
    detections: 4,
    lastSeen: '2026-03-10 11:03',
    area: 'Lilongwe Central',
    trend: 'Active — detected today',
    color: 'var(--red)',
  },
  {
    ...
    score: 78,
    detections: 2,
    lastSeen: '2026-03-10 08:12',
    area: 'Zomba',
    trend: 'Active — detected today',
    color: 'var(--amber)',
  },
  {
    reportId: 'RPT-2026-00045',
    device: 'HP Envy 14',
    score: 61,
    detections: 1,
    lastSeen: '2026-03-10 08:45',
    area: 'Lilongwe',
    trend: 'IoT detection only',
    color: 'var(--amber)',
  },
];
...
export default function ThreatIntelPage() {
  ...
  const TABS = [
    { key: 'hotspots', label: 'Predictive Hotspots', icon: <FiMap />, count: null },
    {
      ...
      count: ANOMALIES.filter((a) => !a.acknowledged).length,
    },
      ...
      {activeTab === 'priority' && (
        <div>
          <div className="alert alert-blue" style={{ marginBottom: 16 }}>
            ...
            <div>
              AI scores each active case 0–100. Score factors: detection frequency, recency, area
              risk level, historical recovery rate. Focus officer resources on the highest-scored
              cases first.
            </div>
          </div>
          ...
          {CASE_SCORES.map((c, i) => (
            <div key={c.reportId} className="card" style={{ marginBottom: 12 }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 16, flexWrap: 'wrap' }}>
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div
                    style={{ fontWeight: 800, fontSize: 15, color: 'var(--ink)', marginBottom: 4 }}
                  >
